            ...
            if (transaction.data.status === 'success') {
                ...
                if (order) {
                    ...
                    if (order.paymentStatus === 'completed') {
                        return res.status(200).json({ message: 'Webhook already processed (duplicate skipped)' });
                    }
                  ...
                  //  await generateQRForTicket(order.ticketId);
                    ...
                    console.warn(`Webhook Warn: Order with ticketCode ${reference} not found in database.`);
                }
            }
        } catch (error) {
